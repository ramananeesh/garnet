...
namespace Garnet.server
{
    public sealed class GarnetStreamManager : IDisposable
    {
        ...
        public unsafe void StreamAdd(ArgSlice keySlice, ArgSlice idSlice, byte* value, int valueLength, int numPairs, ref SpanByteAndMemory output, out byte[] streamKey, out GarnetStreamObject lastStream)
        {
            ...
            try
            {
                ...
                if (!foundStream)
                {
                    ...
                    GarnetStreamObject newStream = new GarnetStreamObject(null, defPageSize, defMemorySize);
        ...
        public unsafe ulong StreamLength(ArgSlice keySlice)
        {
            ...
            if (streams != null)
            {
                bool foundStream = streams.TryGetValue(key, out GarnetStreamObject stream);
                if (foundStream)
                {
        ...
        /// <returns></returns>
        public bool StreamDelete(ArgSlice keySlice, ArgSlice idSlice, out GarnetStreamObject lastSeenStream)
        {
            bool foundStream;
            var key = keySlice.ToArray();
            GarnetStreamObject stream;
